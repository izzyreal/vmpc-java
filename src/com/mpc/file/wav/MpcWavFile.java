...
public class MpcWavFile
{
	...
	private BufferedOutputStream oStream;	// Output stream used for writting data
...
//	private BufferedInputStream biStream;
	...
	public static MpcWavFile newWavFile( int numChannels, long numFrames, int validBits, long sampleRate) throws IOException, WavFileException
	{
		// Instantiate new Wavfile and initialise
		MpcWavFile mpcWavFile = new MpcWavFile();
//		wavFile.file = file;
		mpcWavFile.numChannels = numChannels;
		mpcWavFile.numFrames = numFrames;
		mpcWavFile.sampleRate = sampleRate;
		mpcWavFile.bytesPerSample = (validBits + 7) / 8;
		mpcWavFile.blockAlign = mpcWavFile.bytesPerSample * numChannels;
		mpcWavFile.validBits = validBits;
		...
		mpcWavFile.cache = new CachedOutputStream();
		mpcWavFile.oStream = new BufferedOutputStream(mpcWavFile.cache);
//		wavFile.boStream = new BufferedOutputStream(boStream);
		// Calculate the chunk sizes
		long dataChunkSize = mpcWavFile.blockAlign * numFrames;
		...
		if (dataChunkSize % 2 == 1) {
			mainChunkSize += 1;
			mpcWavFile.wordAlignAdjust = true;
		}
		else {
			mpcWavFile.wordAlignAdjust = false;
		}

		// Set the main chunk size
		putLE(RIFF_CHUNK_ID,	mpcWavFile.buffer, 0, 4);
		putLE(mainChunkSize,	mpcWavFile.buffer, 4, 4);
		putLE(RIFF_TYPE_ID,	mpcWavFile.buffer, 8, 4);

		// Write out the header
		mpcWavFile.oStream.write(mpcWavFile.buffer, 0, 12);

		// Put format data in buffer
		long averageBytesPerSecond = sampleRate * mpcWavFile.blockAlign;

		putLE(FMT_CHUNK_ID,				mpcWavFile.buffer, 0, 4);		// Chunk ID
		putLE(16,							mpcWavFile.buffer, 4, 4);		// Chunk Data Size
		putLE(1,								mpcWavFile.buffer, 8, 2);		// Compression Code (Uncompressed)
		putLE(numChannels,				mpcWavFile.buffer, 10, 2);		// Number of channels
		putLE(sampleRate,					mpcWavFile.buffer, 12, 4);		// Sample Rate
		putLE(averageBytesPerSecond,	mpcWavFile.buffer, 16, 4);		// Average Bytes Per Second
		putLE(mpcWavFile.blockAlign,		mpcWavFile.buffer, 20, 2);		// Block Align
		putLE(validBits,					mpcWavFile.buffer, 22, 2);		// Valid Bits

		// Write Format Chunk
		mpcWavFile.oStream.write(mpcWavFile.buffer, 0, 24);

		// Start Data Chunk
		putLE(DATA_CHUNK_ID,				mpcWavFile.buffer, 0, 4);		// Chunk ID
		putLE(dataChunkSize,				mpcWavFile.buffer, 4, 4);		// Chunk Data Size

		// Write Format Chunk
		mpcWavFile.oStream.write(mpcWavFile.buffer, 0, 8);

		// Calculate the scaling factor for converting to a normalised double
		if (mpcWavFile.validBits > 8)
		{
			// If more than 8 validBits, data is signed
			// Conversion required multiplying by magnitude of max positive value
			mpcWavFile.floatOffset = 0;
			mpcWavFile.floatScale = Long.MAX_VALUE >> (64 - mpcWavFile.validBits);
			System.out.println("floatScale: " + mpcWavFile.floatScale);
		}
		else
		{
			// Else if 8 or less validBits, data is unsigned
			// Conversion required dividing by max positive value
			mpcWavFile.floatOffset = 1;
			mpcWavFile.floatScale = 0.5 * ((1 << mpcWavFile.validBits) - 1);
		}

		// Finally, set the IO State
		mpcWavFile.bufferPointer = 0;
		mpcWavFile.bytesRead = 0;
		mpcWavFile.frameCounter = 0;
		mpcWavFile.ioState = IOState.WRITING;

		return mpcWavFile;
	}

	public static MpcWavFile openWavFile(File file) throws IOException, WavFileException
	{
		// Instantiate new Wavfile and store the file reference
		MpcWavFile mpcWavFile = new MpcWavFile();
		mpcWavFile.file = file;

		// Create a new file input stream for reading file data
		mpcWavFile.iStream = new FileInputStream(file);

		// Read the first 12 bytes of the file
		int bytesRead = mpcWavFile.iStream.read(mpcWavFile.buffer, 0, 12);
		if (bytesRead != 12) throw new WavFileException("Not enough wav file bytes for header");

		// Extract parts from the header
		long riffChunkID = getLE(mpcWavFile.buffer, 0, 4);
		long chunkSize = getLE(mpcWavFile.buffer, 4, 4);
		long riffTypeID = getLE(mpcWavFile.buffer, 8, 4);
...
//			throw new WavFileException("Header chunk size (" + chunkSize + ") does not match file size (" + file.length() + ")");
		...
		while (true)
		{
			...
			bytesRead = mpcWavFile.iStream.read(mpcWavFile.buffer, 0, 8);
			...
			long chunkID = getLE(mpcWavFile.buffer, 0, 4);
			chunkSize = getLE(mpcWavFile.buffer, 4, 4);
			...
			if (chunkID == FMT_CHUNK_ID)
			{
				...
				bytesRead = mpcWavFile.iStream.read(mpcWavFile.buffer, 0, 16);

				// Check this is uncompressed data
				int compressionCode = (int) getLE(mpcWavFile.buffer, 0, 2);
				if (compressionCode != 1) throw new WavFileException("Compression Code " + compressionCode + " not supported");

				// Extract the format information
				mpcWavFile.numChannels = (int) getLE(mpcWavFile.buffer, 2, 2);
				mpcWavFile.sampleRate = getLE(mpcWavFile.buffer, 4, 4);
				mpcWavFile.blockAlign = (int) getLE(mpcWavFile.buffer, 12, 2);
				mpcWavFile.validBits = (int) getLE(mpcWavFile.buffer, 14, 2);

				if (mpcWavFile.numChannels == 0) throw new WavFileException("Number of channels specified in header is equal to zero");
				if (mpcWavFile.blockAlign == 0) throw new WavFileException("Block Align specified in header is equal to zero");
				if (mpcWavFile.validBits < 2) throw new WavFileException("Valid Bits specified in header is less than 2");
				if (mpcWavFile.validBits > 64) throw new WavFileException("Valid Bits specified in header is greater than 64, this is greater than a long can hold");

				// Calculate the number of bytes required to hold 1 sample
				mpcWavFile.bytesPerSample = (mpcWavFile.validBits + 7) / 8;
				if (mpcWavFile.bytesPerSample * mpcWavFile.numChannels != mpcWavFile.blockAlign)
				...
				numChunkBytes -= 16;
				if (numChunkBytes > 0) mpcWavFile.iStream.skip(numChunkBytes);
			}
			else if (chunkID == DATA_CHUNK_ID)
			{
				...
				if (chunkSize % mpcWavFile.blockAlign != 0) throw new WavFileException("Data Chunk size is not multiple of Block Align");

				// Calculate the number of frames
				mpcWavFile.numFrames = chunkSize / mpcWavFile.blockAlign;
			...
			else
			{
				...
				mpcWavFile.iStream.skip(numChunkBytes);
			}
		}
		...
		if (mpcWavFile.validBits > 8)
		{
			...
			mpcWavFile.floatOffset = 0;
			mpcWavFile.floatScale = 1 << (mpcWavFile.validBits - 1);
		}
		else
		{
			// Else if 8 or less validBits, data is unsigned
		...
		mpcWavFile.bytesRead = 0;
		mpcWavFile.frameCounter = 0;
		mpcWavFile.ioState = IOState.READING;
...
//		System.out.println("istream available " + iStream.available());
	...
	public static void main(String[] args)
	{
		...
		try
		{
			...
			{
				MpcWavFile readWavFile = openWavFile(new File(filename));
				readWavFile.display();
